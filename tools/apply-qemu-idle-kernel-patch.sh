...
elif patch --dry-run -l -d "$target" -p1 <"$patch_file" >/dev/null 2>&1; then
	patch -l -d "$target" -p1 <"$patch_file"
elif patch --dry-run -R -l -d "$target" -p1 <"$patch_file" >/dev/null 2>&1; then
	printf 'QEMU idle kernel patch already applied: %s\n' "$target"
else
	printf 'QEMU idle kernel patch does not apply cleanly: %s\n' "$target" >&2
	exit 1
