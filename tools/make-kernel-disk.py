...
def main(kernel_path, disk_path):
    ...
    # Keep the advertised geometry exactly equal to the image size.  The
    # kernel occupies partition a in cylinder 1; cylinder 0 is reserved for
    # the Sun disk label.
    sectors_per_cylinder = partition_sectors
    label = bytearray(SECTOR)
    label[:128] = (f"QEMU test kernel cyl 2 alt 0 hd 1 sec {sectors_per_cylinder}"
