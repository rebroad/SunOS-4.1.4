#ident	"@(#)if_le.c 1.1 94/10/31 SMI"
...
/*
 * Patchable variables controlling collision reduction patch.
 */
#define	LECOLLHIST	1000		/* collision fifo depth */
static	int	lecollenable = 1;	/* enable all the mess below */
static	int	lecollon = 0;		/* collion-reduction "on" */
static	int	lecolltot = 0;		/* # colls in last LECOLLHIST */
static	u_char	lecolltab[LECOLLHIST];	/* colls in last LECOLLHIST packets */
static	int	lecolli = 0;		/* index into lecolltab[] */
static	int	lecollx = 0;		/* # on/off toggles */
static	int	lecolllo = 50;		/* lo threshold */
static	int	lecollhi = 100;		/* hi threshold */
...
#ifndef	sun4c
	/*
	 * Check to see if this an S4DMA based SBus LANCE card by
	 * check for the existance of an ledma or lebuf parent.
	 */
	if (lop == NULL) {
		switch (cpu) {
		case CPU_SUN4M_690:
		case CPU_SUN4M_50:
			/*
			 * S4DMA chip based SBus LANCE cards are not supported
			 * on SS10 and 600 series due to higher SBus latencies
			 * and insufficient local buffering.
			 */
			printf("le%d:  not used - unsupported SBus card!\n",
				unit);
			return;
		}

		/*
		 * Since the proms on these S4DMA based SBus Lance cards
		 * don't present an ledma parent node to the device tree,
		 * we need to mimic the allocation and setup of DVMA
		 * memory done in the ledma driver.
		 */
		if (es->es_membase == NULL) {

			/*
			 * For platforms with an IOMMU, such as those
			 * based on Sun-4M, we allocate some kernel
			 * memory, statically IO map it, and treat it
			 * as a Programmed I/O device (copy to/from)
			 * rather than dynamically IO mapping and
			 * unmapping each buffer to transmit and receive.
			 * This may change if the cost of IO mapping gets
			 * cheaper but unfortunately it's the best tradeoff
			 * for now.
			 *
			 * 'membase' is the CPU-addressable memory base
			 * 'iobase' is the device-addressable memory base
			 */
#define	LEBUFSIZE 128 * 1024

			es->es_membase = kmem_alloc((u_int) LEBUFSIZE);
			if (es->es_membase == NULL) {
				printf("le%d: kmem_alloc(%d) failed\n", unit,
					LEBUFSIZE);
				return (-1);
			}
			/* NOSTRICT */
			es->es_iobase = (caddr_t) mb_nbmapalloc(dvmamap,
						es->es_membase, LEBUFSIZE,
						MDR_BIGSBUS, (int(*)())0,
						(caddr_t) 0);
			if (es->es_iobase == NULL) {
				printf("ledma%d: mb_mapalloc failed\n", unit);
				kmem_free(es->es_membase, (u_int) LEBUFSIZE);
				return (-1);
			}
...
#endif
			es->es_memsize = LEBUFSIZE;
			es->es_flags |= (LE_PIO|LE_IOADDR);
		}
	}
#endif
	...
	int	unit;
{
	...
	while (es->es_if.if_snd.ifq_head) {
		...
		if (lecollenable && lecollon)
			if (es->es_cur_tmd != es->es_nxt_tmd)
				break;
...
leintr()
{
	...
	/*
	 * track # of inits - in case of a giant packet
	 */
	es_init = le_softc[0].es_init;
	...
	for (unit = 0; unit < le_units; unit++, es++) {
		...
		if (csr0 & LE_RINT) {
			...
			for (lmd = es->es_his_rmd;
				 ...
				 * then break out and handle any new
				 * packets in the next interrupt
				 */
				if (es_init < es->es_init)
					break;
...
{
	/*
	 * recalculate the total # collisions over
	 * the previous LECOLLHIST packets.
	 */
	lecolltot = lecolltot - lecolltab[lecolli] + coll;
	lecolltab[lecolli] = coll;
	lecolli++;
	if (lecolli >= LECOLLHIST)
		lecolli = 0;

	/*
	 * Turn the collision-reduction mode "off"
	 * when the number of collisions over the past
	 * LECOLLHIST packets is less than or equal
	 * to the low threshold.
	 */
	if (lecollon && (lecolltot <= lecolllo)) {
		lecollon = 0;
		lecollx++;
	}

	/*
	 * Turn the collision-reduction mode "on"
	 * when the number of collisions over the past
	 * LECOLLHIST packets is greater than or equal
	 * to the high threshold.
	 */
	if ((lecollon == 0) && (lecolltot >= lecollhi)) {
		lecollon = 1;
		lecollx++;
	}
}
	...
	struct le_md	*lmd;
{
	...
	if (!(flags & LMD_STP) && !(flags & RMD_OFLO)) {
		...
		/*
		 * If macio ethernet then do a reset to
		 * prevent the chip receive side from hanging
		 * bug # 1145294
		 */
		if (es->es_leinit && (flags & LMD_ENP)) {
			/*
			 * only need to do this if the packet
			 * is greater than 4096 bytes
			 */
			if (gp_count > lmd->lmd_mcnt)
				leinit(es - le_softc);
			gp_count = 0;
		} else
			gp_count += MAXBUF;
	...
	struct le_md	*lmd;
{
	...
	/*
	 * NCR92C990 can set LCAR on RTRY
	 * AMD7990 does not set LCAR on RTRY
	 */
