#ifndef lint
static char sccsid[] = "@(#)ufs_lockf.c 1.1 94/10/31 SMI";
#endif
	...
	int		class;	/* Lock class (NONE, IO, REMOTE) */
{
	...
	/*
         * l_xxx is set to GRANT_LOCK_FLAG if a previous unlock by lockmgr
         * results in granting other lock mgr results.  lockmgr will call
         * fcntl() repeatedly to get the info of the granted locks when the
         * last granted lock is reported to lockmgr, l_xxx is set to <0.
	 */
	...
	if (lock_debug_on) {
        ...
        /*
         * If the v_filocks list is now null, it's because we recently
         * freed it, so we have to release our reference to the vnode.
         */
        if (vp->v_filocks == NULL) {
                ASSERT(ld->l_xxx != GRANT_LOCK_FLAG);
                VN_RELE(vp);
                if (class == LOCKMGR)
                        ld->l_xxx = LOCKD_LAST_REF;
        } else {
                if (class == LOCKMGR && !has_remote_locks(vp)) {
                        ASSERT(ld->l_xxx != GRANT_LOCK_FLAG);
                        ld->l_xxx = LOCKD_LAST_REF;
                }
        }
	...
	int		class;	/* Lock class, be it I/O, FILE, or LOCKMGR */
{
	...
	lck.length = (ld->l_len == 0) ? LOCK_TO_EOF : ld->l_len;
	lck.base   = (ld->l_start >= 0) ? ld->l_start : 0;
	lck.pid    = ((class == LOCKMGR) ? ld->l_pid : pid);
	lck.class  = 0;
	lck.LockID = LockID++;
	if (!(vp->v_filocks)) {
#ifdef LOCK_DEBUG
	...
	if (ld->l_len == LOCK_TO_EOF) ld->l_len = 0;
        ...
        /*
         * Not quite done, actually.  If this is a lock manager request,
         * check whether there are any remote locks on the file.
         */
        if (class == LOCKMGR && !has_remote_locks(vp)) {
                ld->l_xxx = LOCKD_LAST_REF;
        }
	...
	long	rsys;	/* Remote system ID (if needed) */
{
	...
	struct process_locks	*pl,*plnext;/* Pointer to all locks owned by us */
	...
	for (pl = proc_locks; pl; pl = plnext) {
		plnext=pl->next;
		for (t = pl->lox; t; t = n) {
...
/*
 * Determine whether there are any remote locks for the given vnode.
...
has_remote_locks(vp)
        struct vnode *vp;
{
        struct lock_list *listp = (struct lock_list *)vp->v_filocks;
        struct data_lock *lockp;

        for (lockp = listp->shared; lockp != NULL; lockp = lockp->Next) {
                if (lockp->class == LOCKMGR) {
                        return (1);
                }
        }
        for (lockp = listp->exclusive; lockp != NULL; lockp = lockp->Next) {
                if (lockp->class == LOCKMGR) {
                        return (1);
                }
        }
        for (lockp = listp->pending; lockp != NULL; lockp = lockp->Next) {
                if (lockp->class == LOCKMGR) {
                        return (1);
                }
        }
