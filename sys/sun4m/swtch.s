...
2:
	...
	bnz	2f				! do not bother running them.
	nop

	sethi	%hi(_ufs_lock_released), %g1
	ld	[%g1 + %lo(_ufs_lock_released)], %g2
	tst	%g2				! if klock released in ufs,
	bnz	2f				! do not bother running them.
	nop
#endif	MULTIPROCESSOR
...
4:
	...
	nop; nop; nop			! psr delay, being paranoid
...
2:
	...
	mov	%l0, %psr
	nop				! psr delay
	nop; nop			! add more delay, just in case
...
9:
	...
	andn	%g1, PSR_PIL, %g1
	mov	%g1, %psr
	nop; nop; nop			! psr delay, just in case
 ...
 */
	...
	or	%l4, PSR_PIL, %o5
	mov	%o5, %psr		! spl hi
	nop; nop; nop			! paranoid psr delay
!
