...
For a native-build comparison, push the same source archive into the guest
over the existing serial connection. This keeps the transfer host-to-guest
only and avoids opening a guest-to-host TFTP or FTP data path:

```sh
git archive HEAD | gzip -9 > /var/tmp/sunos-4.1.4-source.tar.gz
./tools/install-file-serial.sh /var/tmp/sunos-4.1.4-source.tar.gz \
...
```sh
git archive HEAD | gzip -9 > /var/tmp/sunos-4.1.4-source.tar.gz
./tools/push-file-ftp.py 10.205.192.4 \
    /var/tmp/sunos-4.1.4-source.tar.gz \
    /home/rebroad/sunos-4.1.4-source.tar.gz
```

The helper contacts the host-facing `10.205.192.4` address, which the host
NATs onto the guest's internal `137.205.192.4` address. It temporarily permits
only the passive FTP data port selected for this transfer and removes that
rule afterwards; it does not create a guest-to-host firewall exception.
